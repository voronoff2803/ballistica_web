// Stub vorbisfile.h for iOS build (standard Vorbis API).
#ifndef VORBISFILE_H_STUB
#define VORBISFILE_H_STUB
...
static vorbis_info _stub_vinfo2 = {44100, 2, {}};
static inline vorbis_info* ov_info(OggVorbis_File*, int) { return &_stub_vinfo2; }
...
static inline long ov_read(OggVorbis_File* vf, char* buf, int len, int, int, int, int*) {
  if (vf && !vf->seekable) {
    vf->seekable = 1;
    int n = len < 4096 ? len : 4096;
    for (int i = 0; i < n; i++) buf[i] = 0;
